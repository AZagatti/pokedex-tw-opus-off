...
Git remote (SSH): `https://github.com/AZagatti/pokedex-tw-opus-off.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-tw-opus-off`. The live URL will be `https://azagatti.github.io/pokedex-tw-opus-off/`.
